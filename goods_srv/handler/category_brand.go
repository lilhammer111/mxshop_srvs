...
func (GoodsServer) CategoryBrandList(c context.Context, r *proto.CategoryBrandFilterRequest) (*proto.CategoryBrandListResponse, error) {
...
func (GoodsServer) GetCategoryBrandList(c context.Context, r *proto.CategoryInfoRequest) (*proto.BrandListResponse, error) {
	brandListResponse := proto.BrandListResponse{}

	var category model.Category
	if result := global.DB.Find(&category, r.Id).First(&category); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "商品分类不存在")
	}

	var categoryBrands []model.GoodsCategoryBrand
	if result := global.DB.Preload("Brands").Where(&model.GoodsCategoryBrand{CategoryID: r.Id}).Find(&categoryBrands); result.RowsAffected > 0 {
...
func (GoodsServer) CreateCategoryBrand(c context.Context, r *proto.CategoryBrandRequest) (*proto.CategoryBrandResponse, error) {
	var category model.Category
	if result := global.DB.First(&category, r.CategoryId); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.InvalidArgument, "商品分类不存在")
	}

	var brand model.Brands
	if result := global.DB.First(&brand, r.BrandId); result.RowsAffected == 0 {
...
func (GoodsServer) DeleteCategoryBrand(c context.Context, r *proto.CategoryBrandRequest) (*emptypb.Empty, error) {
	if result := global.DB.Delete(&model.GoodsCategoryBrand{}, r.Id); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.NotFound, "品牌分类不存在")
	}
	return &emptypb.Empty{}, nil
}

func (GoodsServer) UpdateCategoryBrand(c context.Context, r *proto.CategoryBrandRequest) (*emptypb.Empty, error) {
	var categoryBrand model.GoodsCategoryBrand

	if result := global.DB.First(&categoryBrand, r.Id); result.RowsAffected == 0 {
	...
	categoryBrand.CategoryID = r.CategoryId
	categoryBrand.BrandsID = r.BrandId
